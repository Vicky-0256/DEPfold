...
class RNAfmEmbedding(nn.Module):
       def __init__(
        ...
        self.model,alphabet = fm.pretrained.rna_fm_t12(model_location="pretrained/RNA-FM_pretrained.pth")
        ...
        def forward(self, tokens: torch.Tensor) -> torch.Tensor:
            ...
            tokens = pad(tokens[mask].split(lens.tolist()), self.pad_index, padding_side="right")
            token_mask = pad(mask[mask].split(lens.tolist()), 0, padding_side="right")

            # return the hidden states of all layers
            results = self.model(tokens[:, :self.max_len], need_head_weights=False, repr_layers=[12], return_contacts=False) 
            x = results["representations"][12]
            ...
            for i in range(self.stride, (tokens.shape[1]-self.max_len+self.stride-1)//self.stride*self.stride+1, self.stride):
                results = self.model(tokens[:, i:i+self.max_len], need_head_weights=False, repr_layers=[12], return_contacts=False) 
                y = results["representations"][12]
                x = torch.cat((x, y), 1)
